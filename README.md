...
- **较新版本**（实测 1.2.0604 可用）：应用设置页点「选择并授权目录」，直接唤起系统原生选择器。
- **较旧版本**（实测 1.2.0302 不支持）：进入 **应用中心 → 已安装 → Compare Share → 应用设置 → 访问权限 → 选择允许访问的文件夹**。
  授权后需重启应用（授权信息由系统在启动时注入）。

> **旧版系统的限制**：部分早期 fnOS 版本（如 1.2.0302）不会把目录授权下发给第三方应用，
> 即使已在系统设置中授权也不生效。这是系统侧的能力限制，需要等待系统更新。
> 应用会检测到这种情况并在授权面板中说明；未授权时可继续使用默认收件目录。
